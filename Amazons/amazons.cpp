...
inline double UCB1(node *p){
	return ((p->dep&1)?(p->val):(1-p->val))+C*sqrt(log(p->fa->vis)/p->vis);
}
...
inline bool is_fully_expanded(node *p){
	...
	for(int id=(p->dep&1)?4:0,U=(p->dep&1)?8:4;id<U;id++){
		...
		for(int dir1=0;dir1<8;dir1++){
			...
			for(int nxt_x=cur_x+Dx,nxt_y=cur_y+Dy;!out_of_bound(nxt_x,nxt_y)&&!A[nxt_x][nxt_y];nxt_x+=Dx,nxt_y+=Dy){
				for(int dir2=0;dir2<8;dir2++){
					DX=dx[dir2],DY=dy[dir2];
					for(int arr_x=nxt_x+DX,arr_y=nxt_y+DY;!out_of_bound(arr_x,arr_y)&&(!A[arr_x][arr_y]||cur_x==arr_x&&cur_y==arr_y);arr_x+=DX,arr_y+=DY){
...
inline double calc_M(){
	...
	for(int x,y,i=0;i<4;i++){
		...
		for(int dir=0;dir<8;dir++){
			Dx=dx[dir],Dy=dy[dir];
			for(int _x=x+Dx,_y=y+Dy,len=1;!out_of_bound(_x,_y)&&!A[_x][_y];_x+=Dx,_y+=Dy,len++){
				if(~Queen_2[_x][_y]){
					m+=pow(2,-len)*L[_x][_y];
				}
			}
...
inline void print(){
	for(int i=0;i<SIZE;i++){
		for(int j=0;j<SIZE;j++){
			if(A[j][i]==0)printf(". ");
			else if(A[j][i]==1)printf("B ");
			else if(A[j][i]==2)printf("W ");
			else if(A[j][i]==3)printf("X ");
		}
		puts("");
	}
	puts("");
...
inline double sigma_1(double omega){
	if(omega>BEGIN)return 0.24;
	else if(omega>MIDUM)return 0.60;
	else return 0.90;
}

inline double sigma_2(double omega){
	if(omega>BEGIN)return 0.50;
	else if(omega>MIDUM)return 0.30;
	else return 0.10;
}

inline double sigma_3(double omega){
	if(omega>BEGIN)return 0.26;
	else if(omega>MIDUM)return 0.10;
	else return 0;
}
...
inline void json_test(){
	...
	ret["response"]["x2"]=x_2;
	ret["response"]["y2"]=y_2;
	ret["debug"]["train_times"]=(Json::UInt)TRAIN_TIMES;TRAIN_TIMES=0;
...
inline void local_test(){
	...
	if(MODE==0){
		...
		bool flag=1;
		while(1){
			if(flag)timing=clock()+int(1.95*CLOCKS_PER_SEC),flag=0;
			else timing=clock()+int(0.95*CLOCKS_PER_SEC);
			int T=clock();
			int res=play();
			if(!~res)return;
			trans_policy(res,x_0,y_0,x_1,y_1,x_2,y_2);
			fprintf(stderr,"%d %d %d %d %d %d    %.3lf\n",x_0,y_0,x_1,y_1,x_2,y_2,(double)(clock()-T)/CLOCKS_PER_SEC);//print();
		}
	}
	else{
		int x_0,y_0,x_1,y_1,x_2,y_2;
		...
		trans_policy(play(),x_0,y_0,x_1,y_1,x_2,y_2);print();
		printf("%d %d %d %d %d %d\n",x_0,y_0,x_1,y_1,x_2,y_2);
		while(scanf("%d%d%d%d%d%d",&x_0,&y_0,&x_1,&y_1,&x_2,&y_2)==6){
			play(policy(x_0,y_0,x_1,y_1,x_2,y_2));print();
			trans_policy(play(),x_0,y_0,x_1,y_1,x_2,y_2);
			printf("%d %d %d %d %d %d\n",x_0,y_0,x_1,y_1,x_2,y_2);print();
		}
	}
}
